 ...
 *
 * @param {{templateKey: string, userId: import('mongoose').Types.ObjectId}} opts
 * @returns {Promise<{template: object|null, sender: {name: string, company: string,
 *                    vars: Array<{re: RegExp, value: string}>}}>}
 */
async function loadRenderContext({ templateKey, userId }) {
  const [settings, template] = await Promise.all([
    Settings.findOne({ userId }, { senderName: 1, senderCompany: 1, customVariables: 1 }).lean(),
    Template.findOne({ userId, key: templateKey }, { key: 1, name: 1, subject: 1, body: 1 }).lean(),
  ]);
