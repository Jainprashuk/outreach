...
async function logEvent({ userId = null, category, action, message, meta = {} }) {
  return ActivityLog.create({ userId, category: clean(category), action: clean(action), message: clean(message), meta });
}
...
function auditHttpMutations(req, res, next) {
  // Scheduled sweeps are skipped: they belong to no single account, so the row
  // would be ownerless and invisible in the Logs page. The per-account events
  // they trigger are logged individually, with an owner.
