...
async function scanForBatch(campaignId, want, budget) {
  ...
  while (chosen.length < want) {
    ...
    const hits = (emails.length || sourceIds.length)
      ? await Contact.find({ deleted: { $ne: true }, $or: [
...
async function reconcileDuplicates(campaignId, { budget, batchSize = 500, trigger = 'manual' } = {}) {
  ...
  while (!(budget && budget.expired())) {
    const window = await CampaignRow.find(
      ...
      { campaignId: id, status: 'pending', sourceContactId: null, rowIndex: { $gt: cursor } },
      { email: 1, rowIndex: 1 }
...
async function releaseCampaign(campaignId, { trigger = 'cron', force = false, budget } = {}) {
  ...
  logEvent({ category: 'campaign', action: 'run_started', message: `Campaign run started: ${campaign.name}`, meta: { campaignId: String(id), trigger } })
  ...
  try {
    ...
    const existingContacts = existingRows.length
      ? await Contact.find({ _id: { $in: existingRows.map(r => r.sourceContactId) }, deleted: { $ne: true } }).lean()
      : [];
    ...
    if (missingSources.length) {
      await CampaignRow.updateMany(
        { _id: { $in: missingSources.map(r => r._id) } },
    ...
    await Contact.updateMany(
      { _id: { $in: existingContacts.map(c => c._id) } },
    ...
    if (report.finishedAt && (!report.jobId || report.error)) {
      try {
        await notifyCampaignRun(id, null, report);
        await notifyCampaignCompletion(id);
      } catch (err) {
