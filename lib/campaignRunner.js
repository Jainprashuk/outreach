...
// The workflow runs on cron '5 * * * *' — :05 past every UTC hour — and IST is
// UTC+5:30, so every fire lands at :35 past an IST hour. Mirrors the same
// constant in client/src/lib/campaigns.ts.
const CRON_MINUTE_IST = 35;
 ...
 *               its own ratePerHour
 */
async function buildTimeline({ granularity = 'day' } = {}) {
  const now = Date.now();
  const pastMs = granularity === 'hour' ? 24 * HOUR_MS : 14 * DAY_MS;
  const futureMs = granularity === 'hour' ? 48 * HOUR_MS : 30 * DAY_MS;
  ...
  const doneJobs = await SendJob.find(
    { 'items.processedAt': { $gte: new Date(from) } },
    { items: 1 },
  ).lean();
  ...
  for (const c of campaigns) {
    ...
    let day = Date.UTC(istNow.getUTCFullYear(), istNow.getUTCMonth(), istNow.getUTCDate(),
      c.runHourIst || 0, CRON_MINUTE_IST, 0) - IST_OFFSET_MS;
