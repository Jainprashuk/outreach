...
async function scanForBatch(campaignId, want, budget) {
  ...
  while (chosen.length < want && scanned < scanCap && !(budget && budget.expired())) {
  ...
  return { chosen, skips, scanned, exhausted };
}
...
async function previewNextBatch(campaignId, { limit } = {}) {
  ...
  const budget = deadline(15_000);
  const [{ chosen, skips, scanned, exhausted }, ctx] = await Promise.all([
...
async function releaseCampaign(campaignId, { trigger = 'cron', force = false, budget } = {}) {
  ...
  try {
    ...
    if (scan.chosen.length === 0) {
      // Nothing usable left. A sheet that ran out is finished, not failed.
      const left = await CampaignRow.countDocuments({ campaignId: id, status: 'pending' });
      const done = left === 0;
      await Campaign.updateOne({ _id: id }, {
        $set: {
          lastReleaseOn: today, lastReleaseAt: new Date(), lastError: null,
