...
const { inCooldown } = require('./cooldown');
const { loadInterviewSets, isInInterview } = require('./interviewGuard');
const { notifyCampaignRun, notifyCampaignCompletion } = require('./campaignNotifications');
 ...
 */
async function claimCampaign(campaignId, today, force, userId) {
...
const releaseLease = async (campaignId, userId) => {
  try {
    await Campaign.updateOne({ _id: oid(campaignId), userId }, { $set: { releaseLockAt: null } });
 ...
 */
async function sweepStaleClaims(campaignId, userId) {
  const stale = new Date(Date.now() - LOCK_TTL_MS);
  const id = oid(campaignId);
  await CampaignRow.updateMany(
    { userId, campaignId: id, status: 'queued', contactId: null, claimedAt: { $lt: stale } },
    { $set: { status: 'pending', releaseId: null, claimedAt: null } }
  );
  await CampaignRow.updateMany(
    { userId, campaignId: id, status: 'queued', contactId: { $ne: null }, claimedAt: { $lt: stale } },
    { $set: { status: 'released' } }
  );
}
 ...
 */
async function scanForBatch(campaignId, want, budget, userId) {
  const id = oid(campaignId);
  const chosen = [];
  const skips = [];
  const seen = new Set();
  ...
  // Loaded once per scan: anyone in the interview pipeline is never released.
  const interviewSets = await loadInterviewSets(userId);
  ...
  while (chosen.length < want) {
    ...
    const window = await CampaignRow.find(
      { userId, campaignId: id, status: 'pending', rowIndex: { $gt: cursor } },
      { name: 1, email: 1, company: 1, role: 1, rowIndex: 1, sourceRow: 1, extras: 1, sourceContactId: 1 }
    ...
    const sourceIds = window.filter(r => r.sourceContactId).map(r => r.sourceContactId);
    const hits = (emails.length || sourceIds.length)
      ? await Contact.find({ userId, deleted: { $ne: true }, $or: [
          ...(emails.length ? [{ email: { $in: emails } }] : []),
          ...(sourceIds.length ? [{ _id: { $in: sourceIds } }] : []),
        ] }, { email: 1, lastSentAt: 1, name: 1, company: 1, role: 1 })
          .collation({ locale: 'en', strength: 2 }).lean()
      : [];
    const taken = new Set(hits.map(c => String(c.email).trim().toLowerCase()));
    const sources = new Map(hits.map(c => [String(c._id), c]));

    for (const r of window) {
      if (chosen.length >= want) break;
      if (r.sourceContactId) {
        const contact = sources.get(String(r.sourceContactId));
        if (!contact)              { skips.push({ row: r, reason: 'source_contact_missing' }); continue; }
        if (inCooldown(contact))   { skips.push({ row: r, reason: 'cooldown' }); continue; }
        if (isInInterview({ id: r.sourceContactId, email: contact.email }, interviewSets)) {
          skips.push({ row: r, reason: 'in_interview' }); continue;
        }
        chosen.push(r);
        continue;
      }
      ...
      if (isInInterview({ email: r.email }, interviewSets)) {
        skips.push({ row: r, reason: 'in_interview' }); continue;
      }
 ...
 */
async function reconcileDuplicates(campaignId, { budget, batchSize = 500, trigger = 'manual', userId = null } = {}) {
  const id = oid(campaignId);
  // The owner is a property of the campaign, never of the caller: everything
  // below must stay inside that one user's rows and contacts.
  const owner = userId
    || (await Campaign.findOne({ _id: id }, { userId: 1 }).lean() || {}).userId
    || null;
  ...
  while (!(budget && budget.expired())) {
    const window = await CampaignRow.find(
      // Contact-backed rows intentionally retain their existing Contact; they
      // are not duplicate spreadsheet rows to reconcile away.
      { userId: owner, campaignId: id, status: 'pending', sourceContactId: null, rowIndex: { $gt: cursor } },
      { email: 1, rowIndex: 1 }
    ...
    const hits = await Contact.find(
      { userId: owner, email: { $in: emails }, deleted: { $ne: true } }, { email: 1 }
    ).collation({ locale: 'en', strength: 2 }).lean();
    ...
    if (dead.length) {
      const res = await CampaignRow.updateMany(
        { _id: { $in: dead }, userId: owner, status: 'pending' },
  ...
  if (marked > 0) {
    const [pending, skipped] = await Promise.all([
      CampaignRow.countDocuments({ userId: owner, campaignId: id, status: 'pending' }),
      CampaignRow.countDocuments({ userId: owner, campaignId: id, status: 'skipped' }),
    ]);
    ...
    await Campaign.updateOne({ _id: id, userId: owner }, {
...
async function previewNextBatch(campaignId, { limit, userId = null } = {}) {
  const campaign = await Campaign.findOne({
    _id: oid(campaignId), ...(userId ? { userId } : {}), deleted: { $ne: true },
  }).lean();
  if (!campaign) return null;
  // Everything below belongs to the campaign's owner, not to whoever asked.
  ...
  const [{ chosen, skips, scanned, exhausted, timedOut, capped }, ctx] = await Promise.all([
    scanForBatch(campaign._id, want, budget, owner),
    loadRenderContext({ templateKey: campaign.templateKey, userId: owner }),
  ]);

  const remainingPending = await CampaignRow.countDocuments({
    userId: owner, campaignId: oid(campaignId), status: 'pending',
  });
...
/**
 * What today already owes Gmail for ONE user, across EVERYTHING — campaigns and
 * manual Step 3 sends alike, because it reads that user's SendJobs rather than
 * campaign bookkeeping. The ~500/day ceiling is per Gmail account, and every user
 * has their own, so the count must never span users.
 *
...
async function sendHeadroom(userId) {
  ...
  const live = await SendJob.find(
    { userId, status: { $in: ['pending', 'processing'] } }, { items: 1 }
  ).lean();
...
async function buildTimeline({ range = '7d', scope = 'all', userId = null } = {}) {
  ...
  let jobFilter = { userId };
  if (scope === 'campaigns') {
    const camps = await Campaign.find({ userId, deleted: { $ne: true } }, { releases: 1 }).lean();
    const ids = new Set();
    for (const c of camps) {
      for (const r of (c.releases || [])) if (r.jobId) ids.add(String(r.jobId));
    }
    // An empty set must match nothing, not everything.
    jobFilter = { userId, _id: { $in: [...ids].map(id => {
  ...
  const campaigns = await Campaign.find(
    { userId, deleted: { $ne: true }, status: 'running' },
...
async function circuitTripped(campaign) {
  ...
  const job = await SendJob.findOne(
    { _id: campaign.lastJobId, userId: campaign.userId }, { items: 1, status: 1 }
  ).lean();
 ...
 */
async function releaseCampaign(campaignId, { trigger = 'cron', force = false, budget, userId = null } = {}) {
  const today = istDateKey();
  const startedAt = new Date();

  const claim = await claimCampaign(campaignId, today, force, userId);
  if (!claim.ok) return { ok: false, reason: 'locked_or_already_released', campaignId: String(campaignId) };

  const campaign = claim.campaign;
  const id = oid(campaignId);
  // Every query below is scoped to the campaign's OWNER, taken from the campaign
  // document itself — a campaign must never pull another user's contacts,
  // templates, settings or send jobs.
  const owner = campaign.userId;
  const report = {
  ...
  logEvent({ userId: owner, category: 'campaign', action: 'run_started', message: `Campaign run started: ${campaign.name}`, meta: { campaignId: String(id), trigger } })
    .catch(err => console.error('Activity log write failed:', err.message));

  try {
    await sweepStaleClaims(id, owner);

    // Fail fast, before anything is created, if the template is gone.
    const ctx = await loadRenderContext({ templateKey: campaign.templateKey, userId: owner });
    if (!ctx.template) {
    ...
    const scan = await scanForBatch(id, want, budget, owner);
    ...
    if (scan.skips.length) {
      await CampaignRow.bulkWrite(scan.skips.map(s => ({
        updateOne: {
          filter: { _id: s.row._id, userId: owner, status: 'pending' },
    ...
    if (scan.chosen.length === 0) {
      const left = await CampaignRow.countDocuments({ userId: owner, campaignId: id, status: 'pending' });
      const done = left === 0;
    ...
    await CampaignRow.updateMany(
      { _id: { $in: scan.chosen.map(r => r._id) }, userId: owner, status: 'pending' },
    ...
    const existingRows = scan.chosen.filter(r => r.sourceContactId);
    const newRows = scan.chosen.filter(r => !r.sourceContactId);
    ...
    const { created } = await importContacts(rows, owner);
    const madeByEmail = new Map(created.map(c => [c.email, c]));
    const existingContacts = existingRows.length
      ? await Contact.find({ _id: { $in: existingRows.map(r => r.sourceContactId) }, userId: owner, deleted: { $ne: true } }).lean()
      : [];
    const existingById = new Map(existingContacts.map(c => [String(c._id), c]));

    const releasedRows = [
      ...newRows.filter(r => madeByEmail.has(r.email)),
      ...existingRows.filter(r => existingById.has(String(r.sourceContactId))),
    ];
    const lostToRace = newRows.filter(r => !madeByEmail.has(r.email));
    const missingSources = existingRows.filter(r => !existingById.has(String(r.sourceContactId)));
    ...
    if (lostToRace.length) {
      await CampaignRow.updateMany(
        { _id: { $in: lostToRace.map(r => r._id) }, userId: owner },
    ...
    if (missingSources.length) {
      await CampaignRow.updateMany(
        { _id: { $in: missingSources.map(r => r._id) }, userId: owner },
        { $set: { status: 'skipped', skipReason: 'source_contact_missing', releaseId: null, claimedAt: null } }
      );
      report.skipped += missingSources.length;
    }
    ...
    await CampaignRow.bulkWrite(releasedRows.map(r => ({
      updateOne: {
        filter: { _id: r._id, userId: owner },
        update: { $set: { contactId: String(r.sourceContactId || madeByEmail.get(r.email)._id) } },
      },
    })), { ordered: false });

    // Spreadsheet rows become ordinary queued contacts. Contact-backed rows
    // deliberately remain `in-campaign` until their campaign job actually sends
    // them, so they cannot be manually queued a second time.
    await Contact.updateMany(
      { _id: { $in: created.map(c => c._id) }, userId: owner },
      {
    ...
    await Contact.updateMany(
      { _id: { $in: existingContacts.map(c => c._id) }, userId: owner },
      { $set: { approvalStatus: 'approved' } },
    );

    const items = releasedRows.map(r => {
      const c = r.sourceContactId ? existingById.get(String(r.sourceContactId)) : madeByEmail.get(r.email);
    ...
    // Credentials are read from the owner's stored settings and copied onto the
    // job, so the Vercel worker that eventually runs it needs no process state.
    const creds = await mailer.getSenderFor(owner);
    const job = await SendJob.create({
      ...
      senderName:        ctx.sender.name || creds.name || '',
      senderAppPassword: creds.appPassword || '',
      sendMode: 'drip',
    ...
    try {
      ...
      await SendJob.updateOne({ _id: job.id, userId: owner }, { status: 'cancelled' });
    ...
    const now = new Date();
    await CampaignRow.updateMany(
      { _id: { $in: releasedRows.map(r => r._id) }, userId: owner },
      { $set: {
          status: 'released',
    ...
    if (report.finishedAt) logEvent({
      userId: owner,
      category: 'campaign', action: 'run_finished',
      message: `Campaign run finished: ${campaign.name} — ${report.released} released, ${report.skipped} skipped${report.error ? ', with an error' : ''}`,
      meta: { campaignId: String(id), released: report.released, skipped: report.skipped, jobId: report.jobId, error: report.error },
    }).catch(err => console.error('Activity log write failed:', err.message));
    // Notifications are deliberately best-effort. A temporary Gmail problem
    // must never pause, fail, or duplicate an outreach campaign.
    // Successful batches notify only after their SendJob has actually sent (or
    // failed) every item. A run with no job / a queue error has no worker to do
    // that, so report it immediately instead.
    if (report.finishedAt && (!report.jobId || report.error)) {
      try {
        await notifyCampaignRun(id, null, report, owner);
        await notifyCampaignCompletion(id, owner);
      } catch (err) {
        console.error(`Campaign notification failed for ${String(id)}:`, err.message);
      }
    }
...
async function runDueCampaigns({ campaignIds = null, trigger = 'cron', force = false } = {}) {
  ...
  // Deliberately NOT scoped to one user: this is the scheduler, and it must find
  // every user's due campaigns. Each campaign's own userId is carried down into
  // releaseCampaign(), which scopes everything from there.
  const due = await Campaign.find(filter, { _id: 1, name: 1, userId: 1 }).sort({ createdAt: 1 }).lean();

  const reports = [];
  const deferred = [];
  for (const c of due) {
    if (budget.expired()) { deferred.push({ campaignId: String(c._id), name: c.name }); continue; }
    reports.push(await releaseCampaign(c._id, { trigger, force, budget, userId: c.userId }));
  }
