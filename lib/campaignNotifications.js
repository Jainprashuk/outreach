...
/** Best-effort operational alert. It never changes campaign delivery state. */
async function notifyCampaignRun(campaignId, report) {
  const campaign = await Campaign.findById(campaignId).lean();
  if (!campaign || campaign.deleted) return;
  const release = (campaign.releases || []).at(-1);
  const text = [
    `Campaign run update: ${campaign.name}`,
    '',
    `Status: ${campaign.status}`,
    `Run date (IST): ${report.releasedOn}`,
    `Queued this run: ${n(report.released)}`,
    `Skipped this run: ${n(report.skipped)}`,
    `Rows scanned: ${n(report.scanned)}`,
    report.jobId ? `Send job: ${report.jobId}` : '',
    report.error ? `Issue: ${report.error}` : '',
    release ? `History entry: ${releaseLine(release)}` : '',
    '',
    'This is an automatic Outreach campaign update.',
  ].filter(Boolean).join('\n');
...
async function notifyCampaignCompletion(campaignId) {
  ...
  const text = [
    ...
    ...jobHistory,
    '',
    'This is the complete scheduling history for this campaign. Any pending send-job items will continue their normal drip schedule.',
  ].join('\n');
...
module.exports = { notifyCampaignRun, notifyCampaignCompletion };
