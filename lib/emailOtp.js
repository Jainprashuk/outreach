  ...
  try {
    const res = await fetch(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        text: `You can now sign in to Outreach.\n\n${link ? link + '/login\n\n' : ''}Use this email address — a six-digit code will be sent to it each time you sign in. There is no password.`,
        html: `<!doctype html><html><body style="margin:0;padding:24px;background:#f6f7f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,sans-serif;color:#111827">
  <div style="max-width:420px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:28px">
    <p style="margin:0 0 14px;font-size:16px;font-weight:600">You can now sign in to Outreach.</p>
    <p style="margin:0 0 18px;font-size:14px;color:#374151">Use this email address. A six-digit code is sent to it each time you sign in — there is no password to remember.</p>
    ${link ? `<p style="margin:0"><a href="${escapeHtml(link)}/login" style="display:inline-block;background:#4f46e5;color:#fff;text-decoration:none;padding:10px 18px;border-radius:8px;font-size:14px;font-weight:600">Sign in</a></p>` : ''}
  </div>
</body></html>`,
        ...(OTP_REPLY_TO ? { reply_to: OTP_REPLY_TO } : {}),
      }),
