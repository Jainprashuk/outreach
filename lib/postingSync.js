...
const sourceKeyFor = (source, boardToken, sourceId) => `${source}:${boardToken}:${sourceId}`;
 ...
 */
async function syncBoard(board, { runStartedAt, deadline: budget, dryRun = false } = {}) {
  ...
  const report = {
    ...
    fetched: 0, filtered: 0, inserted: 0, updated: 0, reopened: 0, closed: 0,
  ...
  const keys = postings.map(p => sourceKeyFor(source, token, p.sourceId));
  ...
  if (dryRun) {
    report.closed = await countStale({ source, boardToken: token, keys });
  ...
  // ── UPSERT PASS ──────────────────────────────────────────────────────────
  if (postings.length) {
    const ops = upsertOps(postings, { source, boardToken: token, boardId, runStartedAt, prevByKey });
    try {
      ...
      report.writeErrors = (err && err.writeErrors && err.writeErrors.length) || postings.length;
  ...
  // ── CLOSE PASS ───────────────────────────────────────────────────────────
  report.closed = await closeStale({ source, boardToken: token, runStartedAt });
 ...
 */
function upsertOps(postings, { source, boardToken, boardId, runStartedAt, prevByKey }) {
...
async function syncAllBoards({ boardIds = null, dryRun = false } = {}) {
  ...
  try {
    reports = await mapLimit(list, CONCURRENCY, (board) =>
      syncBoard(board, { runStartedAt, deadline: budget, dryRun })
    );
...
const zeroTotals = () => ({
  ...
  fetched: 0, inserted: 0, updated: 0, reopened: 0, closed: 0,
});
