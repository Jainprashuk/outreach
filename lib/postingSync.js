 ...
 * @param {{runStartedAt: Date, deadline?: object, dryRun?: boolean, userId?: any}} opts
 * @returns {Promise<object>} BoardReport
 */
async function syncBoard(board, { runStartedAt, deadline: budget, dryRun = false, criteria = null, userId } = {}) {
  ...
  if (budget && budget.expired()) {
    ...
    if (!dryRun) await recordBoardOutcome(board, report, runStartedAt, userId);
    return report;
  }
  ...
  if (dryRun) {
    report.closed = boards.closesPostings(source)
      ? await countStale({ source, boardToken: token, keys: listedKeys, userId })
      : 0;
  ...
  if (excludedKeys.length) {
    try {
      await JobPosting.updateMany(
        { userId, sourceKey: { $in: excludedKeys } },
  ...
  // ── UPSERT PASS ──────────────────────────────────────────────────────────
  if (toStore.length) {
    const ops = upsertOps(toStore, { source, boardToken: token, boardId, runStartedAt, prevByKey, queryTag: queryTagFor(board), userId });
    try {
  ...
  if (report.writeErrors > 0) {
    report.ms = Date.now() - startedAt;
    await recordBoardOutcome(board, report, runStartedAt, userId);
    return report;
  }
  ...
  report.closed = boards.closesPostings(source)
    ? await closeStale({ source, boardToken: token, runStartedAt, userId })
    : 0;
...
function upsertOps(postings, { source, boardToken, boardId, runStartedAt, prevByKey, queryTag = null, userId }) {
  return postings.map((p) => {
    ...
    return {
      updateOne: {
        ...
        // userId is part of the key: two users syncing the same board each own
        // their own row instead of overwriting one another.
        filter: { userId, sourceKey },
        update: {
          $set,
          $setOnInsert: {
            userId,
 ...
 */
async function closeStale({ source, boardToken, runStartedAt, userId }) {
  const out = await JobPosting.updateMany(
    { userId, source, boardToken, listingStatus: 'open', lastSeenAt: { $lt: runStartedAt } },
...
async function countStale({ source, boardToken, keys, userId }) {
  return JobPosting.countDocuments({
    userId, source, boardToken, listingStatus: 'open', sourceKey: { $nin: keys },
  });
}

async function recordBoardOutcome(board, report, runStartedAt, userId) {
  ...
  try {
    await JobBoard.updateOne({ _id: board._id || board.id, userId }, update);
  } catch (_) {
 ...
 *
 * @param {{boardIds?: string[]|null, dryRun?: boolean, userId?: any}} [opts]
 * @returns {Promise<object>} RunReport
 */
async function syncAllBoards({ boardIds = null, dryRun = false, userId } = {}) {
  const runStartedAt = new Date();
  const budget = deadline(RUN_BUDGET_MS);

  // One read per run, shared by every board — it is the same profile for all.
  const settingsDoc = await Settings.findOne({ userId }, { jobCriteria: 1 }).lean();
  const criteria = normaliseCriteria(settingsDoc && settingsDoc.jobCriteria);

  const filter = { userId, deleted: { $ne: true }, enabled: true };
  ...
  if (list.length === 0) {
    return emptyRun(runStartedAt, dryRun ? null : await readLastSyncAt(userId), 'no-boards');
  }
  ...
  let leaseId = null;
  if (!dryRun) {
    const claim = await claimLease(userId);
    if (!claim.ok) {
      return {
    ...
    previousSyncAt = await readLastSyncAt(userId);
  }

  let reports;
  try {
    reports = await mapLimit(list, CONCURRENCY, (board) =>
      syncBoard(board, { runStartedAt, deadline: budget, dryRun, criteria, userId })
    );
    ...
    if (!dryRun && leaseId) {
      await Settings.updateOne({ _id: leaseId, userId }, { $set: { lastPostingSyncAt: runStartedAt } });
    }
  } finally {
    if (!dryRun && leaseId) await releaseLease(leaseId, userId);
  }
...
async function claimLease(userId) {
  // getForUser() first, so this user's document is guaranteed to exist and every
  // write can be scoped to its _id. Do NOT upsert on Settings: with no unique
  // index, a filter that fails to match (i.e. the lock IS held) would insert a
  // SECOND settings document for the user — which breaks the one-doc-per-user
  // invariant and silently defeats the lease, because each contender then owns
  // its own copy. The lease is per user: one user's sync never blocks another's.
  const singleton = await Settings.getForUser(userId);
  const stale = new Date(Date.now() - LOCK_TTL_MS);

  // projection excludes the resume Buffer — MANDATORY, the settings doc holds one.
  ...
  if (before === null) {
    const held = await Settings.findOne({ _id: singleton._id, userId },
...
const releaseLease = async (id, userId) => {
  try {
    await Settings.updateOne({ _id: id, userId }, { $set: { postingSyncLockAt: null } });
  } catch (_) { /* the TTL is the real backstop */ }
};

const readLastSyncAt = async (userId) => {
  const s = await Settings.findOne({ userId }, { lastPostingSyncAt: 1 }).lean();
