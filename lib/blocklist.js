...
/** Loads one user's blocklist as two Sets, for cheap repeated lookups within one job run. */
async function loadBlocklistSets(userId) {
  const entries = await Blocklist.find({ userId }).lean();
