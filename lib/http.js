...
/**
 * One JSON POST, sharing `fetchJson`'s attempt/retry machinery.
 *
 * NEVER throws — same tagged result as `fetchJson`, plus `errorBody`/`retryAfter` on a
 * non-2xx.
 *
 * The defaults are deliberately the opposite of `fetchJson`'s: no retries, and nothing in
 * `retryOn`. The one caller is the reply classifier, where a 429 means "this provider is
 * out of quota, go ask the next one" — sleeping 250ms and re-asking the same exhausted
 * provider would just burn the request budget to learn the same thing twice. A caller that
 * wants the backoff can still pass `retries`.
 *
 * @param {string} url
 * @param {object} [opts]
 * @param {any} [opts.body]              JSON-serialisable request body
 * @param {number} [opts.timeoutMs]      per-attempt timeout
 * @param {number} [opts.maxBytes]       abort past this many bytes
 * @param {number} [opts.retries=0]      additional attempts after the first
 * @param {number[]} [opts.retryOn=[]]   status codes worth retrying
...
async function postJson(url, {
  ...
  const bodyText = JSON.stringify(body === undefined ? {} : body);
  let attempts = 0;
  let last = null;
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    attempts++;
    last = await attemptOnce(url, { timeoutMs, maxBytes, signal, headers, method: 'POST', bodyText });
...
async function attemptOnce(url, { timeoutMs, maxBytes, signal, headers, method = 'GET', bodyText }) {
  ...
  if (!res.ok) {
    // `errorBody` is how a caller tells apart two 4xx that mean opposite things — an LLM
    // provider's "quota exceeded" (wait, or fail over) from "bad request" (never retry).
    // Truncated because a provider's HTML error page is not worth carrying around.
    return { ok: false, status: res.status, data: null, bytes: body.bytes, tooLarge: false,
             error: `HTTP ${res.status}`, errorBody: (body.text || '').slice(0, 500),
...
module.exports = { fetchJson, postJson, mapLimit, deadline, USER_AGENT, DEFAULT_MAX_BYTES };
