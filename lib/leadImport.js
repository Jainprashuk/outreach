...
async function importLeads(source, { ignoredRows = 0, batchUpdatedAt = null, userId } = {}) {
  // 1. explode by email
  const exploded = source.flatMap(explodeLead)
    .map(r => ({ ...r, userId, dedupeKey: dedupeKeyFor(r), batchUpdatedAt }));
  ...
  const stored = await Lead.find(
    { userId, dedupeKey: { $in: unique.map(r => r.dedupeKey) }, ...BASE_FILTER },
  ...
  for (const r of unique) {
    ...
    if (merged.length !== (existing.queries || []).length) {
      backfill.push({ updateOne: { filter: { _id: existing._id, userId }, update: { $set: { queries: merged } } } });
    }
  }
